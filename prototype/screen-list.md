...
| R-F | 紧急事件已留存线索 | `#/rider/emergency-confirm` | 应急卡（四个号码）、平台留存记录、为什么单独一页 | 查看留存线索、返回首页 |

> R-F 为第二轮新增：紧急事件提交后不进入普通工单进度视图，避免「紧急事件仍走普通流程」的口径矛盾。
> R-B 第二轮新增「数据用途与单独同意」入口与弹层（三项勾选，含位置信息单独同意）；R-C 新增「修改描述」与 AI 生成内容标识、申诉入口。
...
| G-D | 核心指标概览 | `#/gov/metrics` | 「原型演示口径」与「仿真实验口径（第一轮结果 + 95% 区间）」两区、分群体参与率 | 查看口径说明与待补材料 |

## 4 仿真端屏幕

| 编号 | 屏幕 | 路由 | 关键元素 | 主要动作 |
| --- | --- | --- | --- | --- |
| S-A | 情景对比 | `#/sim/compare` | S0–S3 情景卡、数据来源横幅、六项指标对比表（含 95% 区间）、分群体参与率、配对效应表、辅助指标、待补材料 | 切换情景、查看区间与配对效应 |
| S-B | 参数与假设 | `#/sim/config` | 参数表（值 / 来源 / 影响情景）、情景新增机制、**敏感性分析档位（未执行）**、机制风险 | 阅读参数来源与单变量限制 |
| S-C | 结果局限 | `#/sim/limits` | 已完成/未完成清单、待补材料（`SIM_RESULT_NEEDED_*`）、结果局限、产品局限 | 阅读 |
...
| `screenshots/10-gov-metrics.png` | G-D | 核心指标概览（原型演示口径 + 仿真实验口径） |
| `screenshots/11-sim-compare.png` | S-A | S0–S3 六项指标对比（第一轮真实结果 + 95% 区间） |
| `screenshots/12-sim-config.png` | S-B | 参数与假设（含未执行的敏感性档位） |
| `screenshots/13-sim-limits.png` | S-C | 结果局限与待补材料 |
| `screenshots/14-rider-emergency-confirm.png` | R-F | 紧急事件已留存线索（不进入普通工单流程） |

截图分辨率 1440×1000（设备像素比 2，便于报告排版）。第二轮共 **14 张**，全部由 `node prototype/app/shot.js` 重新渲染。
